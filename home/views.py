...
def sendsms(phone,password):   
    ...
    auth_token = 'fc046fb8dc1529a3af584fc5964eb66c'
    client = Client(account_sid, auth_token)
    client.messages \
            .create(
                    body="welcom in omoma clinic your password is : "+str(password),
                    from_='+18482835968',
                    to='+201159087296'
                    )
...
def create_profile(request):
    ...
    try:   
                ...
                if request.POST.get('form_type')=='adduser':                                #2
                                    ...
                                    if form.is_valid():  
                                        ...
                                        try: 
                                            ...
                                            form=CreateProfileForm(request.POST)                        #7
                                            ...
                                            user.save()
                                            try:
                                                profile_create=Profile.objects.create(user=user,name=name,mobile=username) #9
                                            except:
                                                form=CreateProfileForm(request.POST)                                       #10
                                            else:           
                                                profile_create.save()                                                      #11
                                                sendsms(phone=username,password=password)
                                                profiles=getprofile(request)                                               #12
                                                return render(request,'home.html',{'form1':form,'profiles':profiles})      #13      
                                    else:
                                        form=CreateProfileForm(request.POST)              #13
                elif request.POST.get('form_type')=='search':                             #14
                   form=CreateProfileForm()                                               #15
        ...
        return render(request,'home.html',{'form1':form,'profiles':profiles})     #16 
...
def profileupdate(request,unique_id):
    ...
    try:
       ...
       user=User.objects.get(id=profile.user_id)                                    #2
    except:
        return redirect('home:home')
    else:                                                                           #3
            if profile and user:  
...
def addvisiting(request,unique_id):
    ...
    '''
    try:
       pro=Profile.objects.get(unique_id=unique_id)                                    #1
    except:
...
def homeuser(request):
        if request.method=="GET":
          visting=Visiting.objects.filter(profile_id=Profile.objects.get(user_id=request.user.id))
          serializer=VisitingSerializers(visting,many=True)  
...
only user login
rest api 
get all image visiting that user login 
pass visiting.uuid to url to get all images of that visiting
return Response list of Json

'''
@api_view(['GET','POST'])
@login_required(login_url='login')
def image_visiting(request,unique_id):
       if request.method=="GET":
